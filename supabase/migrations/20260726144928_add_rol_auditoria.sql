-- Nuevo rol "auditoria" (Gerencia/Auditoría): de solo consulta, igual que
-- contador — ve todas las agencias pero no puede registrar ni aprobar nada.
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_rol_check;
ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_rol_check
  CHECK (rol IN ('pendiente', 'admin', 'responsable', 'contador', 'auditoria'));

-- Se reemplazan las políticas de solo lectura total para incluir también a
-- "auditoria" junto a "admin" y "contador".
...
CREATE POLICY "movimientos select" ON public.movimientos FOR SELECT
  USING (public.get_my_role() IN ('admin', 'contador', 'auditoria') OR agencia_id = public.get_my_agencia());
...
CREATE POLICY "items select" ON public.movimiento_items FOR SELECT
  USING (EXISTS (
    ...
    AND (public.get_my_role() IN ('admin', 'contador', 'auditoria') OR m.agencia_id = public.get_my_agencia())
  ));
...
CREATE POLICY "reembolsos select" ON public.reembolsos FOR SELECT
  USING (
    public.get_my_role() IN ('admin', 'contador', 'auditoria')
